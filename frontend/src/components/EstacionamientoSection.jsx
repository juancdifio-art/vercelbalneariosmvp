...
function EstacionamientoSection({
  ...
  return (
    <div className="rounded-xl bg-sky-50 border border-cyan-100 px-3 py-3 sm:px-4 sm:py-4 text-sm">
      <p className="text-slate-900 font-medium mb-1">Estacionamiento</p>
      ...
      <div className="mt-2 rounded-xl border border-slate-300 bg-white max-h-[600px] overflow-y-auto overflow-x-auto">
        {(() => {
          const totalPlazas = Number.parseInt(
            establishment?.parkingCapacity ?? '0',
            10
          );

          if (!totalPlazas || Number.isNaN(totalPlazas)) {
            return (
              <div className="px-4 py-6 text-[11px] text-slate-600">
                Configurá primero la cantidad de plazas de estacionamiento en la sección de configuración del establecimiento.
              </div>
            );
          }

          const today = new Date();
          const days = Array.from({ length: 30 }, (_, i) => addDays(today, parkingDayOffset + i));
                      ...
                      {days.map((day, idx) => {
                        ...
                        if (isReserved) {
                          groupForCell = reservationGroups.find(
                            (g) =>
                              g.serviceType === 'parking' &&
                              g.resourceNumber === plazaNumero &&
                              g.status === 'active' &&
                              g.startDate <= dateStr &&
                              g.endDate >= dateStr
                          );
                          ...
                          if (groupId !== null) {
                            colorIndexForCell = getGroupColorIndex(groupId, 'parking');
                          }
                        }
...
export default EstacionamientoSection;
