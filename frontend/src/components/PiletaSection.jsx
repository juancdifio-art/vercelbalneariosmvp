  ...
  const stats = useMemo(() => {
    ...
    poolGroups.forEach((g) => {
      ...
      if (start <= todayStr && end >= todayStr) {
        const adults = Number.parseInt(String(g.poolAdultsCount ?? g.adultsCount ?? '0'), 10) || 0;
        const children = Number.parseInt(String(g.poolChildrenCount ?? g.childrenCount ?? '0'), 10) || 0;
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {filteredAndSortedGroups.map((group) => {
                  ...
                  const adults = Number.parseInt(String(group.poolAdultsCount ?? group.adultsCount ?? '0'), 10) || 0;
                  const children = Number.parseInt(String(group.poolChildrenCount ?? group.childrenCount ?? '0'), 10) || 0;
